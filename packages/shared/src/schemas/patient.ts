...
export const patientSchema = z
  .object({
    ...
    chiefComplaint: z.string().max(2000).nullable(),
    pastDentalHistory: z.string().max(4000).nullable(),
    medicationsUsing: z.string().max(2000).nullable(),
...
export const createPatientSchema = z.object({
  ...
  pastDentalHistory: z.string().trim().min(1, "Past dental history is required").max(4000),
  medicationsUsing: z.string().trim().min(1, "Medications is required").max(2000),
